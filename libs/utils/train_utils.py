...
def save_checkpoint(state, is_best, file_folder,
    ...
    if is_best:
        ...
        # if os.path.exists(os.path.join(file_folder, 'model_best.pth.tar')):
        #     os.remove(os.path.join(file_folder, 'model_best.pth.tar'))
        # os.symlink(os.path.join(file_folder, file_name), os.path.join(file_folder, 'model_best.pth.tar'))
...
def make_optimizer(model, optimizer_config):
    ...
    decay = set()
    no_decay = set()
    whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv1d, MaskedConv1D, torch.nn.ConvTranspose1d)
